<!--
  VENDORED, HISTORICAL, IMMUTABLE.

  This file is RFC UCAL-1 as issued, reproduced verbatim. Do not edit it to
  fix an error. Errors are recorded in spec/SPEC-DELTAS.md and corrected in
  spec/UCAL-1.1.md; this file exists so that both have something to be
  measured against.
-->
...
> - For **what changed and why**, read [`SPEC-DELTAS.md`](SPEC-DELTAS.md).
> - For the **rule vocabulary** the implementation cites, read
>   [`RULES.md`](RULES.md).
>
> This file is kept because a correction is only meaningful against the text it
> corrects. Section and rule numbering here is authoritative for both documents:
> the implementation cites it 494 times across 75 sections, so UCAL-1.1 amends
> the text in place and never renumbers it.
...
pub trait Body {
    ...
    fn obliquity(&self)      -> Option<RatedParam>;
...
fn derive_leap_rule(solar_day: &Ratio<Ticks>, orbital_period: &Ratio<Ticks>,
                    max_drift: &Delta) -> Result<LeapRule, BodyError>;
```
...
Algorithm: for each satellite, compute the synodic period relative to the body's solar day exactly as `1 / |1/P_orb − 1/P_solar_day|`; admit the satellite only if that period lies within `bounds` (default 5–100 solar days, D-11); for each admitted satellite, expand `orbital_period / synodic_period` as a continued fraction and return the convergents as candidate cycle-to-year commensurability rules.
...
DATUM (declared, in ticks)
  ...
  ORIGIN_OFFSET     = 9 304 311 741 502 590 385 × BEAT
                    = 8070204002895596515944343085635637180530466139316558837890625
                      (203 bits; 88 base-5 digits; 62 trailing base-5 zeros)
  DOMAIN            = [0, 2^512)
...
Full-width encode is 44 `divmod` steps. Digit-by-digit division by 5 MUST NOT be used.
